...
namespace Task4
{
    class Program
    {
        static void Main()
        {
            StreamReader sr = new StreamReader("pupils.txt");
            byte n = Convert.ToByte(sr.ReadLine());
            Pupil[] pupils = new Pupil[n];
            byte[] max = new byte[] { 20, 20, 20 };
            for (int i = 0; i < n; i++)
            {
                string[] str = sr.ReadLine().Split(' ');
                pupils[i].Surname = str[0];
                pupils[i].Name = str[1];
                pupils[i].Sum = (byte)(Convert.ToByte(str[2]) + Convert.ToByte(str[3]) + Convert.ToByte(str[4]));
                if (pupils[i].Sum<max[0])
                {
                    max[2] = max[1];
                    max[1] = max[0];
                    max[0] = pupils[i].Sum;
                }
                else if (pupils[i].Sum < max[1])
                {
                    max[2] = max[1];
                    max[1] = pupils[i].Sum;
                }
                else if (pupils[i].Sum < max[2])
                {
                    max[2] = pupils[i].Sum;
                }
            }
